...
public class MformulaVisitorFlink implements MformulaVisitor<DataStream<PipelineEvent>> {
    ...
    public DataStream<PipelineEvent> visit(MAnd f) {
        ...
        DataStream<PipelineEvent> input1duplicated = input1
                .flatMap(new DuplicateTerminators(Main.numberProcessors))
                .setParallelism(1);
        DataStream<PipelineEvent> input2duplicated = input2
                .flatMap(new DuplicateTerminators(Main.numberProcessors))
                .setParallelism(1);
        // partition data
        ConnectedStreams<PipelineEvent, PipelineEvent> connectedStreams = input1.partitionCustom(new Partitioner<Integer>() {
            @Override
            public int partition(Integer key, int numPartitions) {
                ...
                .connect(
                        input2.partitionCustom(new Partitioner<Integer>() {
                    @Override
                    public int partition(Integer key, int numPartitions) {
                        return key % numPartitions;
                    }}, new KeySelector<PipelineEvent, Integer>() {
